 ...
 * Fetches the latest phone number information and updates the restaurant record
 */
export async function GET(req: NextRequest) {
  try {
    ...
    if (!firmId) {
      return NextResponse.json({ error: 'Missing firmId (restaurantId)' }, { status: 400 });
    }
    ...
    console.log('[Telephony Refresh] Fetching phone number from Vapi:', restaurant.vapi_phone_number_id);

    // Fetch phone number details from Vapi
    try {
      const getResponse = await vapi.get(`/phone-number/${restaurant.vapi_phone_number_id}`);
      ...
      // Update restaurant record if number changed
      if (phoneNumber !== restaurant.inbound_number_e164) {
      ...
      return NextResponse.json({
        success: true,
        phoneNumber,
        vapiPhoneNumberId: restaurant.vapi_phone_number_id,
