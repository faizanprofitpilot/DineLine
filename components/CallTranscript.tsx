...
export default function CallTranscript({ call }: CallTranscriptProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-auto">
      {/* Header */}
      <div className="p-6 border-b border-[#DEB887] bg-white sticky top-0 z-10">
        ...
        <h1 className="text-2xl font-bold mb-1" style={{ color: '#654321' }}>
          Call Transcript
        </h1>
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <div className="text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: '#A0522D' }}>
                Status
              </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 h-2 bg-[#DEB887] rounded-full overflow-hidden" style={{ maxWidth: '80px' }}>
                  <div
        ...
        {summary && (
          <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
            <button
              ...
              className="w-full p-4 flex items-center justify-between hover:bg-[#FFF0C2] transition-colors"
            >
              <span className="font-semibold" style={{ color: '#654321' }}>Call Review</span>
              {reviewOpen ? <ChevronUp className="w-5 h-5" style={{ color: '#A0522D' }} /> : <ChevronDown className="w-5 h-5" style={{ color: '#A0522D' }} />}
            </button>
            {reviewOpen && (
              <div className="p-4 border-t border-[#DEB887] space-y-4">
                <div>
                  <h4 className="text-sm font-semibold mb-2" style={{ color: '#654321' }}>
                    {summary.title}
                  </h4>
                  <ul className="list-disc list-inside space-y-1 text-sm" style={{ color: '#A0522D' }}>
                ...
                {summary.follow_up_recommendation && (
                  <div>
                    <h4 className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#A0522D' }}>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
          ...
          >
            <div className="flex items-center gap-2">
              <span className="font-semibold" style={{ color: '#654321' }}>Full Transcript</span>
              {call.transcript_text ? (
                <span className="text-xs px-2 py-1 rounded-full bg-green-50 text-green-700">Available</span>
              ) : (
                <span className="text-xs px-2 py-1 rounded-full bg-[#FFF0C2] text-[#A0522D]">Not available</span>
              )}
            </div>
            {transcriptOpen ? <ChevronUp className="w-5 h-5" style={{ color: '#A0522D' }} /> : <ChevronDown className="w-5 h-5" style={{ color: '#A0522D' }} />}
          </button>
          {transcriptOpen && (
            <div className="p-6 border-t border-[#DEB887]">
                {transcriptTurns.length > 0 ? (
                    ...
                    {transcriptTurns.map((turn, idx) => (
                      <div key={idx} className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="text-xs font-semibold mb-1" style={{ color: '#A0522D' }}>
                            {turn.role === 'assistant' ? 'AI Receptionist' : 'Caller'}
                          </div>
                          <div className="text-sm leading-relaxed" style={{ color: '#654321' }}>
                            {turn.content}
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="text-sm text-center py-8" style={{ color: '#A0522D', opacity: 0.8 }}>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
          <button
            ...
            className="w-full p-4 flex items-center justify-between hover:bg-[#FFF0C2] transition-colors"
          >
            <span className="font-semibold" style={{ color: '#654321' }}>Call Analytics</span>
            {analyticsOpen ? <ChevronUp className="w-5 h-5" style={{ color: '#A0522D' }} /> : <ChevronDown className="w-5 h-5" style={{ color: '#A0522D' }} />}
          </button>
          {analyticsOpen && (
            <div className="p-4 border-t border-[#DEB887]">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="capitalize" style={{ color: '#654321' }}>
