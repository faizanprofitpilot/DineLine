  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white rounded-xl shadow-sm" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }}>
        <CardHeader>
          <CardTitle style={{ color: '#8B4513' }}>Current Subscription</CardTitle>
          <CardDescription style={{ color: '#A0522D' }}>
          ...
          {hasActiveSubscription ? (
            <>
              <div className="grid gap-6 md:grid-cols-2">
                <div>
                  <div className="text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: '#A0522D' }}>
                    Current Plan
                  </div>
                  <div className="text-2xl font-bold" style={{ color: '#8B4513' }}>
                    {PLAN_DETAILS[currentPlan]?.name || 'Unknown'}
                  </div>
                  <div className="text-sm mt-1" style={{ color: '#A0522D' }}>
              ...
              {!firm.subscription_cancel_at_period_end && (
                <div className="border-t border-gray-200 pt-6">
                  <div className="text-xs font-semibold uppercase tracking-wide mb-4" style={{ color: '#A0522D' }}>
                    Change Plan
                  </div>
                        ...
                        <div
                          ...
                          className={`p-4 rounded-lg border-2 ${
                            isCurrentPlan
                              ...
                              : 'border-[#DEB887] hover:border-[#FF8C42]'
                          }`}
                        >
                          <div className="font-semibold mb-1" style={{ color: '#8B4513' }}>
                            {planDetail.name}
                          </div>
                          <div className="text-sm mb-3" style={{ color: '#A0522D' }}>
                            {planDetail.price}/month
                          </div>
                          {isCurrentPlan ? (
                            <div className="text-sm font-medium" style={{ color: '#8B4513' }}>Current Plan</div>
                          ) : (
                            <Button
            ...
            <div className="text-center py-8">
              <p className="text-sm mb-4" style={{ color: '#A0522D' }}>
                No active subscription. Subscribe to a plan to start using DineLine.
              </p>
              <Button
                asChild
      ...
      {hasActiveSubscription && (
        <Card className="bg-white rounded-xl shadow-sm" style={{ boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06)' }}>
          <CardHeader>
            <CardTitle style={{ color: '#8B4513' }}>Billing History</CardTitle>
            <CardDescription style={{ color: '#A0522D' }}>
          ...
          <CardContent>
            {loadingInvoices ? (
              <div className="flex items-center justify-center py-8">
                <Loader2 className="h-6 w-6 animate-spin" style={{ color: '#A0522D' }} />
              </div>
            ) : invoices.length === 0 ? (
              <div className="text-center py-8">
                <FileText className="h-12 w-12 mx-auto mb-4" style={{ color: '#A0522D', opacity: 0.5 }} />
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {invoices.map((invoice) => (
                      <tr key={invoice.id} className="border-b border-[#DEB887] hover:bg-[#FFF0C2] transition-colors">
                        <td className="px-4 py-4 text-sm" style={{ color: '#8B4513' }}>
                          {invoice.number || invoice.id.slice(-8)}
                        </td>
                        <td className="px-4 py-4 text-sm" style={{ color: '#A0522D' }}>
                          {formatDate(invoice.created)}
                        </td>
                        <td className="px-4 py-4 text-sm font-medium" style={{ color: '#8B4513' }}>
