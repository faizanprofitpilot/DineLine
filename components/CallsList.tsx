...
export default function CallsList({ calls, searchParams }: CallsListProps) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="overflow-auto">
        {!calls || calls.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-sm" style={{ color: '#A0522D', opacity: 0.8 }}>No calls found.</p>
          </div>
        ) : (
          <table className="w-full">
            <thead className="bg-[#FFF0C2] sticky top-0">
            ...
            <tbody>
              {calls.map((call) => {
                ...
                return (
                  <tr
                    key={call.id}
                    className="border-b border-[#DEB887] transition-colors hover:bg-[#FFF0C2] cursor-pointer"
                    onClick={() => router.push(`/calls/${call.id}`)}
                  >
                    <td className="px-4 py-3 text-sm" style={{ color: '#654321' }}>
      ...
      {/* Pagination */}
      {calls && calls.length > 0 && (
        <div className="p-6 border-t border-[#DEB887] flex items-center justify-between text-sm" style={{ color: '#A0522D' }}>
          <div>
            Items per page: 10
          </div>
          <div>
